...
const SignUp = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (signUpValidate()) {
            ...
            fetch('https://my-brand-aimelive.herokuapp.com/api/v1/users/signup', {
                ...
                .catch((err) => {
                    setError('Error ' + err.message + '! check your network connection and try again')
                    setIsCreating('Try Again')
                })
        }
    }
    function signUpValidate() {
        ...
        if (niceEmail.test(email)) {
            document.querySelector("#userEmail").focus()
            setError('Invalid Email: should not contain any special character except @ _ .')
            return false
        }
        if (phone === "") {
    ...
    return (<div>
        <div className="login">
            ...
            {error && <div className="divErrorSignUp"><div><i className="fa fa-warning"></i> {error} </div></div>}
            {created && <div className="userCreated">{created} Redirecting...</div>}
            <form className="login_form" name="loginAdmin" onSubmit={handleSubmit}>
                ...
                <input type="text"
                    ...
                    value={phone}
                    onChange={(e) => {
                        if (!(/[abcdefghijklmnopqrstuvwxyz!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/).test(e.target.value)) {
                ...
                />
                <br />
                <input type="submit" value={isCreating} placeholder="Email or Phone number" className="input submit" /><br />
